class Country < ActiveRecord::Base
  ...
  def full_descriptor
    ...
    cities_w_time_zone = City.find(:all, :select => "distinct time_zone_id, utf8_name, region_id", :conditions => "cities.country_id = #{self.id} AND time_zone_id IS NOT NULL")
    ...
    if (cities_w_time_zone) # Make sure there are any cities in this country
      ...
      cities_w_time_zone.each do |city_i|
        if (city_i.time_zone)  # Watch out as NULL is a distinct time zone!! 
          country_city_tz.push("#{city_i.utf8_name}, #{city_i.region.name}, #{self.name} (#{city_i.time_zone.name})") 
        end
      end
